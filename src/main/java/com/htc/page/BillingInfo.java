...
public class BillingInfo {
	   private WebDriver driver;
		...
		public void billingInfo(Map <Object,Object>mapData)  {
			...
			act.moveToElement(company).click().keyDown(Keys.CONTROL).sendKeys("a").sendKeys("x").keyUp(Keys.CONTROL).sendKeys(mapData.get("cmpnt").toString()).build().perform();
			
			act.moveToElement(StreetAddress).click().keyDown(Keys.CONTROL).sendKeys("a").sendKeys("x").keyUp(Keys.CONTROL).sendKeys(mapData.get("address").toString()).build().perform();
			
			act.moveToElement(StreetAddress2).click().keyDown(Keys.CONTROL).sendKeys("a").sendKeys("x").keyUp(Keys.CONTROL).sendKeys(mapData.get("street").toString()).build().perform();
			
			act.moveToElement(city).click().keyDown(Keys.CONTROL).sendKeys("a").sendKeys("x").keyUp(Keys.CONTROL).sendKeys(mapData.get("city").toString()).build().perform();
			
			act.moveToElement(postcode).click().keyDown(Keys.CONTROL).sendKeys("a").sendKeys("x").keyUp(Keys.CONTROL).sendKeys(mapData.get("zipcode").toString()).build().perform();
			country3.click();
			
			 act.moveToElement(fax).click().keyDown(Keys.CONTROL).sendKeys("a").sendKeys("x").keyUp(Keys.CONTROL).sendKeys(mapData.get("fax").toString()).build().perform();
			 ShipAddress.click();
			 Continue.click();
